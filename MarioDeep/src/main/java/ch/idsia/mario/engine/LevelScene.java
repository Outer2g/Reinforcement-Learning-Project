...
//                }
    ...
    public void init()
    {
        try
        {
            //FileInputStream level = (FileInputStream) LevelScene.class.getResourceAsStream("resources/tiles.dat");
            FileInputStream level = new FileInputStream("resources/tiles.dat");

            Level.loadBehaviors(new DataInputStream(level));
        }
        catch (IOException e)
        {
            System.out.println("Couldn't open resources/");
            e.printStackTrace();
            System.exit(0);
        }
        /*        if (replayer!=null)
